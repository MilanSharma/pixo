...
export default function ChatScreen() {
    ...
    useEffect(() => {
        ...
        if (UUID_REGEX.test(otherUserId)) {
             channel = supabase.channel(`chat:${user.id}:${otherUserId}`)
    ...
    const loadChatData = async () => {
        ...
        try {
            if (UUID_REGEX.test(otherUserId)) {
                ...
                if (mockUser) {
                    setOtherUser({ id: mockUser.id, username: mockUser.username, avatar_url: mockUser.avatar });
                    ...
                    if (stored) {
                        ...
                        setMessages([{ id: 'm1', text: `Hi! This is ${mockUser.username}.`, sender: 'them', time: 'Now' }]);
                    }
                }
            }
        } catch (error) {
    ...
    const renderItem = ({ item }: { item: Message }) => (
        <View style={[styles.messageContainer, item.sender === 'me' ? styles.myMessage : styles.theirMessage]}>
            <Text style={[styles.messageText, item.sender === 'me' ? styles.myMessageText : styles.theirMessageText]}>{item.text}</Text>
            <Text style={[styles.timeText, item.sender === 'me' ? styles.myTimeText : styles.theirTimeText]}>{item.time}</Text>
        </View>
    );
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0}>
                <View style={[styles.inputContainer, { paddingBottom: Math.max(insets.bottom, 10) }]}>
                    ...
                    <Pressable onPress={handleSend} style={styles.sendButton} disabled={!inputText.trim()}><Send size={20} color="#fff" /></Pressable>
                </View>
...
const styles = StyleSheet.create({
    ...
    myMessage: { alignSelf: 'flex-end', backgroundColor: Colors.light.tint, borderBottomRightRadius: 4 },
    theirMessage: { alignSelf: 'flex-start', backgroundColor: '#f5f5f5', borderBottomLeftRadius: 4 },
    ...
    inputContainer: { flexDirection: 'row', alignItems: 'flex-end', padding: 12, borderTopWidth: 1, borderTopColor: '#f0f0f0', backgroundColor: '#fff' },
    input: { flex: 1, minHeight: 40, maxHeight: 100, backgroundColor: '#f5f5f5', borderRadius: 20, paddingHorizontal: 16, paddingTop: 10, paddingBottom: 10, marginRight: 10, fontSize: 15 },
    sendButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: Colors.light.tint, justifyContent: 'center', alignItems: 'center', marginBottom: 0 },
});
